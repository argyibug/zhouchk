...
(function() {
  ...
  const FOOTER_SOCIALS = [
    {
      href: 'https://github.com/argyibug',
      label: 'GitHub',
    ...
    {
      href: 'https://www.linkedin.com/in/chengkang-zhou-54811a204/',
      label: 'LinkedIn',
    ...
    {
      href: 'https://scholar.google.com/citations?user=C50jgToAAAAJ&hl=zh-TW&oi=sra',
  ...
  function getFooterCopy() {
    ...
    if (getCurrentPage() === 'photography') {
      return '© 2026 Chengkang Zhou. All photographs are original works.';
    }

    return '© 2026 Chengkang Zhou';
  }
  ...
  function buildNav() {
    ...
    const navLinks = NAV_ITEMS.map((item) => {
      ...
      const opensInNewTab = item.key === 'cv' && /\.pdf$/i.test(item.href);
      const targetAttr = opensInNewTab ? ' target="_blank" rel="noopener"' : '';
      return `<a href="${toRootHref(item.href)}" class="nav-button"${ariaCurrent}${targetAttr}>${item.label}</a>`;
    }).join('');
